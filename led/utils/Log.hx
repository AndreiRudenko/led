...
class Log {
	...
	macro public static function assert(expr:Expr, ?reason:ExprOf<String>) {
			...
			return macro @:pos(Context.currentPos()) {
				if(!$expr) throw led.utils.Log.DebugError.assertion( '$_str' + $reason);
			}
		#end
		return macro null;
	} //assert
	...
	macro public static function assertnull(value:Expr, ?reason:ExprOf<String>) {
			...
			return macro @:pos(Context.currentPos()) {
				if($value == null) throw led.utils.Log.DebugError.null_assertion('$_str was null' + $reason);
			}
		#end
		return macro null;
	} //assert
